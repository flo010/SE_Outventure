...
public class HikeDetailServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        request.getRequestDispatcher("/hike/hike_detail/hike_detail.jsp").forward(request, response);
    }

    private ArrayList<ArrayList<String>> getWeatherList(double latitude, double longitude) {
        Client client = ClientBuilder.newClient();
        String url = "https://api.openweathermap.org/data/2.5/forecast/daily?lat=" + latitude + "&lon=" + longitude + "&cnt=4&units=metric&appid=" + APIKeys.getWeatherApiKey();
